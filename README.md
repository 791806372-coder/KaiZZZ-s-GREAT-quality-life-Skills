# KaiZZZ-s-GREAT-quality-life-Skills

这个仓库用于收集可复用的 Codex skills，覆盖内容反馈、前端视觉效果和高频个人工作流。

## Skills

- `daily-share-feedback`: 生成可直接发群的「总结 / 点评 / 建议」。
- `live-share-feedback`: 生成主持人现场可口播的短点评。
- `share-feedback-review`: 生成更完整的私下复盘点评。
- `cyberpunk-glitch-button`: 生成可复用的 CSS 赛博朋克 glitch 按钮效果。
- `knowledge-html-site`: 生成高质量、可溯源的知识内容 HTML 站点。

## 建议用法

```text
Use $daily-share-feedback to 根据这场分享转写，写一版可以直接发群的总结、点评和建议。
```

```text
Use $live-share-feedback to 根据这场分享内容，写一句主持人现场可以说的点评。
```

```text
Use $share-feedback-review to 根据这场分享转写，写一份给同学看的复盘点评。
```

```text
Use $cyberpunk-glitch-button to 给这个 HTML 页面加一个赛博朋克 glitch CTA 按钮效果。
```

```text
Use $knowledge-html-site to 把这个知识主题和资料包做成一个高质量 HTML 知识站。
```
